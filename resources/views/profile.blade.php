...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Profile - {{ config('app.name', 'Genje') }}</title>
        ...
        <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
            <div class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A] p-6 sm:p-8">
                ...
                <div class="space-y-6">
                    <!-- Profile Picture -->
                    <div>
                        <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Profile Picture</label>
                        <div class="flex items-center gap-4">
                            @if(auth()->user()->avatar)
                                <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->name }}" class="w-20 h-20 border border-[#e3e3e0] dark:border-[#3E3E3A] object-cover">
                            @else
                                <div class="w-20 h-20 bg-[#1b1b18] dark:bg-white text-white dark:text-[#1b1b18] flex items-center justify-center font-medium border border-[#e3e3e0] dark:border-[#3E3E3A]">
                                    {{ auth()->user()->initials() }}
                                </div>
                            @endif
                        </div>
                    ...
                    <!-- Name -->
                    <div>
                        <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Name</label>
                        <p class="text-[#1b1b18] dark:text-[#EDEDEC]">{{ auth()->user()->name }}</p>
                    </div>

                    <!-- Email -->
                    <div>
                        <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Email</label>
                        <p class="text-[#1b1b18] dark:text-[#EDEDEC]">{{ auth()->user()->email }}</p>
                    </div>

                    <!-- Provider -->
                    <div>
                        <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Login Provider</label>
                        <p class="text-[#1b1b18] dark:text-[#EDEDEC] capitalize">{{ auth()->user()->provider }}</p>
                    </div>
                </div>
            </div>
        </main>
