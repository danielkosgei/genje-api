...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $article->title }} - {{ config('app.name', 'Genje') }}</title>
    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] min-h-screen">
        ...
        <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
            <!-- Back Button -->
            <a href="{{ route('home') }}" class="inline-flex items-center gap-2 text-sm text-[#706f6c] dark:text-[#A1A09A] hover:text-[#1b1b18] dark:hover:text-[#EDEDEC] mb-6 transition-colors">
                ← Back to news
            </a>

            <!-- Article -->
            <article class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A]">
                @if($article->image_url)
                <div class="w-full h-64 sm:h-96 bg-gray-200 dark:bg-[#2a2a2a] bg-cover bg-center" style="background-image: url('{{ $article->image_url }}');"></div>
                @endif
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <!-- Title -->
                    <h1 class="text-3xl sm:text-4xl font-semibold mb-4 text-[#1b1b18] dark:text-[#EDEDEC]">{{ $article->title }}</h1>
                    ...
                    <div class="mt-8 pt-6 border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                        <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">By {{ $article->author }}</p>
                    </div>
                    @endif

                    <!-- Original Link -->
                    <div class="mt-6">
                        <a href="{{ $article->url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-6 py-3 bg-[#1b1b18] dark:bg-white text-white dark:text-[#1b1b18] hover:opacity-90 transition-opacity font-medium">
            ...
            <div class="mt-12">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <a href="{{ route('article', $relatedArticle->id) }}" class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A] hover:shadow-lg transition-shadow">
                        @if($relatedArticle->image_url)
                        <div class="w-full h-32 sm:h-40 bg-gray-200 dark:bg-[#2a2a2a] bg-cover bg-center" style="background-image: url('{{ $relatedArticle->image_url }}');"></div>
                        @endif
                        <div class="p-4">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-xs font-medium text-[#706f6c] dark:text-[#A1A09A]">{{ $relatedArticle->source }}</span>
                            ...
                            <h3 class="text-base font-semibold mb-2 line-clamp-2">{{ $relatedArticle->title }}</h3>
                            <p class="text-xs text-[#706f6c] dark:text-[#A1A09A] line-clamp-2">{{ $relatedArticle->description }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
